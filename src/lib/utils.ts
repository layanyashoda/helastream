...
export function triggerCallbackOnClickOrOnKeydown(
...
export function getAttribute(
  ...
  const element = (target as HTMLElement).closest(closest);
...
export function getCompactNotation(value: string | number) {
...
export function getTitleWithSeasonNumber(season: number, title: string) {
  if (season <= 0) return title;

  return `S${season}: ${title}`;
}

export function getTitleWithSeasonAndEpisodeNumber(
  season: number,
  episode: string,
  title: string,
  join: string = " - ",
...
export function getLocaleDate(dateString: string | Date, locale?: string) {
...
export function getReadableDate(dateString: string | Date) {
...
export function timeToFormattedTime(seconds: number): string {
...
export function secondsToFormattedSeconds(seconds: number): string {
